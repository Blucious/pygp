            ...
            for page_id, page_url, is_retry in page_iter:
                ...
                if mp.no_such_topic:
                    ...
                    if no_topic_found_cnt > self.NO_TOPIC_FOUND_UPPERLIMIT:
                        __log__.debug("[+]Procedure exit, 'no_topic_found_cnt' > {0}".format(self.NO_TOPIC_FOUND_UPPERLIMIT))
                        break
                    __log__.debug('[+]No topic found {0}, count {1}'.format(page_url, no_topic_found_cnt))
                elif mp.reserved:
